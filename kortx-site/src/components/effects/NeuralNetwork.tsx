...
interface Node {
  ...
  baseRadius: number;
  pulsePhase: number;
  layer: number; // 0 = background dim, 1 = mid, 2 = foreground bright
  energy: number; // 0-1, how "activated" the node is
  targetEnergy: number;
}
...
export function NeuralNetwork({
  ...
  useEffect(() => {
    ...
    const initNodes = () => {
      ...
      nodesRef.current = Array.from({ length: particleCount }, () => {
        ...
        const baseRadius = layer === 2 ? 2.5 + Math.random() * 1.5 : layer === 1 ? 1.5 + Math.random() * 1 : 1 + Math.random() * 0.5;
        return {
    ...
    const spawnPulse = (nodes: Node[]) => {
      ...
      // Find nodes near the mouse to originate pulses from
      ...
      // Pick a random pair of connected near nodes
      ...
      pulsesRef.current.push({
        ...
        color: Math.random() > 0.7 ? "#00FFAA" : particleColor,
      });
    };
    ...
    const animate = () => {
      ...
      // Spawn data pulses periodically when mouse is active
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (mouse.active) {
          ...
          if (dist < mouseRadius) {
            ...
            // Set target energy based on proximity
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (distance < connectionDistance) {
            ...
            // Base opacity depends on layer and energy
            ...
            if (maxEnergy > 0.3) {
              // Energized connection - glowing gradient line
      ...
      // Draw data pulses (flowing dots along connections)
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        const layerAlpha = node.layer === 2 ? 0.9 : node.layer === 1 ? 0.6 : 0.35;
        const alpha = layerAlpha * brightness;

        if (node.energy > 0.5) {
          // High energy - white core with colored edge
          const gradient = ctx.createRadialGradient(
            node.x, node.y, 0,
    ...
    // Use the parent element for mouse tracking so canvas can keep pointer-events-none
  ...
  }, [particleCount, connectionDistance, particleColor, mouseRadius, hexToRgb]);
